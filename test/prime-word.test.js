...
describe('PrimeWords', () => {
  ...
  it ('Expect Mock words are string', () => {
    for (let index in Mock) {
      expect( (typeof Mock[index]) ).to.be.equal('string')
    }
  })
  ...
  it ('Expect primeWord isPrime returns value type to be equal a boolean', () => {
    expect( (typeof primeWord.isPrime(Mock.primeWord)) ).to.be.equal('boolean')
    expect( (typeof primeWord.isPrime(Mock.nonPrimeWord)) ).to.be.equal('boolean')
  })

  it ('Expect Mock primeWord to be equal a true', () => {
    expect(primeWord.isPrime(Mock.primeWord)).to.be.equal(true)
  })

  it ('Expect Mock nonPrimeWord to be equal a false', () => {
    expect(primeWord.isPrime(Mock.nonPrimeWord)).to.be.equal(false)
  })
})
